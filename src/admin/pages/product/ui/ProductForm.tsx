...
// Update the path below to the actual location of your product.interface file
// For example, if it's in src/interfaces/product.interface.ts, use:
// Update the path below to the actual location of your product.interface file
// For example, if it's in src/interfaces/product.interface.ts, use:
import type { Product, Size } from '@/api/interfaces/product.interface';
...
export const ProductForm = ({ title, subTitle, product, onSubmit, isPending }: Props) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFiles((prev) => [...prev, ...Array.from(files)]);
    const currentFiles = getValues('files') || [];  
    setFiles((files) => [...currentFiles, ...Array.from(files)]);
  };
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  />
                  {
                    errors.description && (
                    <p className="text-red-500 text-sm">
                      { errors.description.message || 'La descripción es requerida.'}
                    </p>
                    )
                  } 
                </div>
